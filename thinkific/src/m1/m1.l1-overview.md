[🔗 For translation, open lesson in new tab and use Chrome translate](https://langchain-ai.github.io/lca-deepagents/m1/m1.l1-overview.html)
...
buildSlideshow({
  ...
  slides: [
    ...
    {
      src: 'images/m1.l2-whatis-4.svg',
      tag: 'Step 4 of 4 · Filesystem',
      caption: `One of those built-ins is a filesystem the agent can read from and write to (ls, write_file, and friends). It uses it as a scratchpad for notes and results on a long task. A later lesson covers it in depth; for now, just know it's there.`
    },
  ]
});
</script>
...
<p align="center">
  <img src="images/m1.l2-system-prompt-code.svg" alt="A faded create_deep_agent code snippet with two lines highlighted: model=model is labeled 'the model, the LLM engine' and system_prompt=... is labeled 'the system prompt, the text the model reads on every call (the USER part)'." width="700">
</p>

Those two are the pieces you control most directly, so they're where this module
lives. You'll swap the model and watch the built-in scaffolding stay put, then add
