...
<svg viewBox="0 0 860 400" role="img" aria-label="Step one shows turn one's prompt, a System Prompt, Tools, and Skills stack totaling 5,200 tokens, plus 3 new tokens for the message Good morning. Step two adds a Snapshot arrow to a second box where that same stack, now 5,203 tokens, is tagged CACHED, plus 3 new tokens for What's the weather. Step three annotates the first box: 5,200 tokens billed at 100% cost, the first and only time those tokens are billed full price. Step four annotates the remaining two facts: 6 tokens, the always-fresh per-turn messages that are never cached, and 5,203 tokens billed at 10% cost, read from cache on every later turn." xmlns="http://www.w3.org/2000/svg" font-family="'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif">
  ...
  <g id="dg-cache-tokens-s0" class="step step-hidden">
    <line x1="235" y1="170" x2="280" y2="170" stroke="#7FC8FF" stroke-width="1.5" marker-end="url(#cache-arrow)"/>
    <text x="257" y="158" text-anchor="middle" font-size="10.5" fill="#7FC8FF" font-family="'IBM Plex Mono', ui-monospace, monospace">Snapshot</text>

    <g id="dg-cache-tokens-fade">
      ...
      <rect x="304" y="254" width="202" height="32" rx="5" fill="#0F2A1E" stroke="#34D399" stroke-width="1"/>
      <text x="405" y="275" text-anchor="middle" font-size="12" fill="#CCE9FF">Good morning!</text>
    </g>
  ...
  <g id="dg-cache-tokens-s1" class="step step-hidden">
    <line x1="520" y1="110" x2="545" y2="92" stroke="#7FC8FF" stroke-width="1" stroke-dasharray="3 3"/>
    <rect x="545" y="68" width="275" height="48" rx="6" fill="#161F34" stroke="#7FC8FF" stroke-width="1.2"/>
    <text x="560" y="87" font-size="13" font-weight="600" fill="#F2FAFF">5,200 tokens</text>
    <text x="560" y="104" font-size="10.5" fill="#99D3FF" font-family="'IBM Plex Mono', ui-monospace, monospace">Billed at 100% cost &#183; first time only</text>
  </g>

  <g id="dg-cache-tokens-s2" class="step step-hidden">
    <line x1="520" y1="189" x2="545" y2="174" stroke="#34D399" stroke-width="1" stroke-dasharray="3 3"/>
    <rect x="545" y="150" width="275" height="48" rx="6" fill="#0F2A1E" stroke="#34D399" stroke-width="1.2"/>
    <text x="560" y="169" font-size="13" font-weight="600" fill="#E8FFF3">5,203 tokens</text>
    <text x="560" y="186" font-size="10.5" fill="#34D399" font-family="'IBM Plex Mono', ui-monospace, monospace">Billed at 10% cost &#183; read from cache</text>

    <polyline points="500,345 520,345 545,280" fill="none" stroke="#5BADDF" stroke-width="1" stroke-dasharray="3 3"/>
    <text x="560" y="270" font-size="12.5" font-weight="600" fill="#CCE9FF">6 tokens</text>
    <text x="560" y="288" font-size="10.5" fill="#5BADDF" font-family="'IBM Plex Mono', ui-monospace, monospace">always fresh, both turns combined</text>
  </g>
  </g>
...
<div class="sd-bar">
  <div class="sd-caption">
    <strong id="dg-cache-tokens-tag">Every turn starts full price</strong>
    <span id="dg-cache-tokens-cap">A fresh request reprocesses the whole prompt: the system prompt, tools, and skills, plus whatever's new this turn, here "Good morning!".</span>
  </div>
...
(function () {
  var steps = [
    { tag: 'Every turn starts full price', caption: 'A fresh request reprocesses the whole prompt: the system prompt, tools, and skills, plus whatever\'s new this turn, here "Good morning!".' },
    { tag: 'Snapshot the prefix', caption: 'After the response comes back, the provider snapshots that prefix into the cache, now including the tokens that were new this turn.' },
    { tag: 'First time, full price', caption: 'The very first time those 5,200 tokens are processed, before any cache exists, they\'re billed at the regular input rate: 100% cost.' },
    { tag: 'Cached prefix, discounted', caption: 'From the next turn on, that same prefix, now 5,203 tokens, is read from cache at a fraction of the price, about 10% here. Only the newest message each turn, 6 tokens total across both turns, is billed at full price every time.' }
  ];
  var id = 'dg-cache-tokens';
  var s0 = document.getElementById(id + '-s0');
  var s1 = document.getElementById(id + '-s1');
  var s2 = document.getElementById(id + '-s2');
  var fadeEl = document.getElementById(id + '-fade');
  ...
  if (!s0 || !s1 || !s2 || !stage) return;
  var current = 0;

  function render() {
    s0.classList.toggle('step-hidden', current < 1);
    s1.classList.toggle('step-hidden', current < 2);
    s2.classList.toggle('step-hidden', current < 3);
    if (fadeEl) { fadeEl.style.opacity = current >= 2 ? '0.55' : '1'; }
...
Run the butler script from Lab 1 twice in a row, unchanged:
