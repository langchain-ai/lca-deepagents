...
Lab 1 had you swap between a handful of premade personas (pirate, cowboy, Shakespeare) on top of the butler system prompt already in that file. This homework asks you to write an entirely new persona from scratch, for a domain of your own choosing: a sports coach, a noir detective, a kindergarten teacher, a support bot for a made-up company, whatever you like. There's no single correct persona here, that's the point.

Open `python/m1/m1.4_homework.py`.

1. **TODO 1**: write your own `SYSTEM_PROMPT` string that gives the agent a persona and a domain of your choosing (not a butler, not pirate, cowboy, or Shakespeare from Lab 1's examples).
2. **TODO 2**: invoke the agent with at least two different test prompts and print both responses, so you can see the persona hold up across more than one question.
...
Stuck, or want to see the finished shape before you start? `m1.4_homework_filled.py` is a reference copy with every TODO filled in (using an over-enthusiastic middle-school science teacher persona); run it the same way to see what "done" looks like end to end.
