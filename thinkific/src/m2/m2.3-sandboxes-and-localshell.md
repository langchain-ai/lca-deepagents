...
Use `virtualMode: true` when you want filesystem tools (`ls`, `read_file`, `write_file`, etc.) to be path-scoped under `rootDir`. Either way, `rootDir` does not restrict the `execute` tool. Shell commands run with host permissions and can access paths outside the filesystem-tool root. That's why LocalShellBackend is unsuitable for production or untrusted input.
...
## Run Your Own Task in a Sandbox
...
Lab 1 wired up a sandboxed coding assistant for one fixed task: writing and running a Fibonacci script. This homework asks you to do the same thing, but for a task of your own choosing.

Open `python/m2/m2.3_homework.py`.

1. **TODO 1**: write a system prompt for your sandboxed agent, giving it a persona or a set of working rules, as long as it tells the agent to write code to a file before running it.
2. **TODO 2**: write the user message describing your own task: any code or shell command you want the agent to write, save, and execute inside the sandbox.
...
Stuck, or want to see the finished shape before you start? `m2.3_homework_filled.py` is a reference copy with every TODO filled in (using a dice-rolling simulation and a curious-lab-assistant persona); run it the same way to see what "done" looks like end to end.
...
Lab 1 wired up a sandboxed coding assistant for one fixed task: writing and running a Fibonacci script. This homework asks you to pick your own pair of tasks for the same sandbox to run, one after another, so you can see that the sandbox's filesystem sticks around between `invoke()` calls instead of resetting each time.

Open `typescript/m2/m2.3_homework.ts`.

1. **TODO 1**: write a system prompt for your sandboxed agent, giving it a persona or a set of working rules, as long as it tells the agent to write code to a file before running it.
2. **TODO 2**: write two task messages for the same agent/sandbox. `TASK_ONE` should have the agent write and run code that saves a file; `TASK_TWO` must depend on that file already being there (read it, extend it, reuse a value from it) without recreating it, so it only succeeds if the sandbox actually kept `TASK_ONE`'s state around.
...
Stuck, or want to see the finished shape before you start? `m2.3_homework_filled.ts` is a reference copy with every TODO filled in (using a random-numbers-then-stats task pair and a curious-lab-assistant persona); run it the same way to see what "done" looks like end to end.
