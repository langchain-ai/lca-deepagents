...
<MCQ
    ...
    choices='["Ephemeral task state that will only be needed during the current session", "API credentials and secrets the agent needs to call external services", "Project-specific instructions, stored preferences, and context the agent should recall across sessions", "The agent\'s tool definitions and allowed-tools list"]'
    correctIndex={2}
