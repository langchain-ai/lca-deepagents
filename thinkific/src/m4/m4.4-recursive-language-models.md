...
(function () {
  ...
  function render() {
    ...
    if (callC) {
      callC.style.opacity = (current === 0 || current === 2) ? '0' : '1';
      callC.querySelector('rect').setAttribute('stroke-width', current === 3 ? '2.5' : '1.5');
    }
    if (lineA2C) {
...
Compare that to the diagram above: `Promise.all` over `pages` *is* the `for` loop on the right. The `summarizer` subagent is a subagent the way `genre-researcher` was, just dispatched by a loop in code instead of by the model turn by turn.
...
- Deep Agents implements this as **dynamic subagents**: subagents (m4) dispatched from a script running in a **code interpreter** (m2.4). See the naming note above on how this differs from the RLM paper itself.
