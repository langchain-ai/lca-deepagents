// typescript/m5/sales_assistant_sandbox/test_diagnostic.ts
/**
...
async function testSandboxWriteAndExecute(client: Client): Promise<Result> {
  // No local outputs/ directory to inspect in sandbox mode — the sandbox's
  // filesystem only exists remotely, so verification has to go back through
  // the agent itself rather than checking disk directly.
  const label = "Sandbox — write_file + execute round-trip in /outputs";
  process.stdout.write(`  Running: ${label}... `);
  try {
    const thread = await client.threads.create();
    await askInThread(
      ...
      thread.thread_id,
      context`
        Use write_file to save the text "DIAG-SANDBOX-OK" to
        /outputs/diag-sandbox-test.txt, then use execute to run
        \`cat /outputs/diag-sandbox-test.txt\` and tell me exactly what it printed.`
    );
    const [reply] = await askInThread(
      client,
      thread.thread_id,
      "What did the cat command print, verbatim?"
    );
    const passed = reply.includes("DIAG-SANDBOX-OK");
    console.log("done");
    return { label, status: passed ? "PASS" : "FAIL", detail: shorten(reply, 80) };
  } catch (exc) {
    console.log("done");
...
async function testNewsletterAsyncTask(client: Client): Promise<Result> {
  const label = "newsletter-agent — async task launches in the background";
  process.stdout.write(`  Running: ${label}... `);
  if (!process.env.TAVILY_API_KEY) {
    console.log("done");
    return { label, status: "SKIP", note: "TAVILY_API_KEY not set — newsletter subagent disabled" };
  }
  try {
    const thread = await client.threads.create();
    const [reply] = await askInThread(
      client,
      thread.thread_id,
      'Write this week\'s "This Week in Music" newsletter for the Rock and Jazz genres.'
    );
    const state = await client.threads.getState(thread.thread_id);
    const asyncTasks = (state.values as { asyncTasks?: Record<string, unknown> }).asyncTasks ?? {};
    const passed = Object.keys(asyncTasks).length > 0;
    console.log("done");
    return {
      label,
      status: passed ? "PASS" : "FAIL",
      detail: passed ? `task(s): ${Object.keys(asyncTasks).join(", ")}` : shorten(reply, 80),
    };
  } catch (exc) {
    console.log("done");
    return { label, status: "FAIL", detail: String(exc) };
...
async function main(): Promise<void> {
  ...
  if (serverResult.status === "FAIL") {
    ...
    const rest = [
      ...
      testInboxManager,
      testHitlInterrupt,
      testSandboxWriteAndExecute,
      testNewsletterAsyncTask,
    ];
