...
# happens to exist on this machine (~/Documents/Github/deep-agents-ui). It
# has no async-subagent or real-sandbox file support, so the
# newsletter/async-task/sandbox-files features won't show up here.
...
# keep paying for sandbox compute until idleTtlSeconds catches up.
cleanup() {
    ...
    npx tsx "$SCRIPT_DIR/stop-sandboxes.ts" || true
}
