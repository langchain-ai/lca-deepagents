// typescript/m5/sales_assistant_sandbox/subagents.ts
/**
 ...
 *
 * genre-researcher itself is still an in-process (synchronous) subagent, but
 * not of the main agent — it's used inside newsletter-agent, a standalone
 * graph (see `newsletter-agent.ts`) launched via `createAsyncSubAgentMiddleware`
 * so the whole newsletter job (research + assembly) runs in the background
...
export const GENRE_PROMPT = context`
...
export function buildSubagents(
  ...
  { mailTools }: BuildSubagentsOptions
): SubAgent[] {
  ...
  return [chinookAnalyst, quoteReviewer, inboxManager];
}
